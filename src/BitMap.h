...
class BitMap {
    ...
    public:
        ...
        std::vector<int> getPixel(int i,int j);
...
vector<int> BitMap::getPixel(int x, int y) {
    if(x<m_width && y<m_height) {
	//std::cerr << "GETPIXEL" << x << " " << y <<endl;
        std::vector<int> v(3,0);
        y = m_height - 1 - y; 
        v[0] = (int) ( m_pixelData[ m_rowSize*y+3*x+2 ] ); //red
        v[1] = (int) ( m_pixelData[ m_rowSize*y+3*x+1 ] ); //greed
        v[2] = (int) ( m_pixelData[ m_rowSize*y+3*x+0 ] ); //blue
...
void BitMap::swapPixel(int i, int j, int i2, int j2) {
    std::vector<int> p1 = (*this).getPixel(i,j);

    std::vector<int> p2 = (*this).getPixel(i2,j2);
